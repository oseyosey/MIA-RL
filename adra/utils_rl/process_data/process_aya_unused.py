"""
Process and clean the Aya unused dataset from hamishivi/tulu-3-unfiltered.

This script filters for the ai2-adapt-dev/tulu_v3.9_aya_unused subset and applies
filtering criteria similar to process_tulu_aya.py:
1. Filters for dataset == "ai2-adapt-dev/tulu_v3.9_aya_unused"
2. Filters for single-turn conversations (len(messages) == 2)
3. Filters examples based on token count bounds:
   - user message: 100-500 tokens (default)
...
Usage:
    python process_aya_unused.py --output-dir /path/to/output
    
    # With custom token bounds
    python process_aya_unused.py --output-dir /path/to/output \
        --user-min-tokens 100 --user-max-tokens 500 \
...
def process_aya_unused_dataset(
    dataset_path: str = "hamishivi/tulu-3-unfiltered",
    dataset_split: str = "train",
    dataset_filter: str = "ai2-adapt-dev/tulu_v3.9_aya_unused",
    ...
    """
    Process and clean the Aya unused dataset from tulu-3-unfiltered.
    
    Args:
        dataset_path: Path to the tulu-3-unfiltered dataset on HuggingFace Hub
        dataset_split: Dataset split to load
        dataset_filter: Dataset value to filter for (default: ai2-adapt-dev/tulu_v3.9_aya_unused)
    ...
    print(f"Filtering for dataset: {dataset_filter}")
    
    # Process each example
    processed_examples = []
    stats = {
        'total': 0,
        'filtered_wrong_dataset': 0,
    ...
    for i, example in enumerate(dataset):
        ...
        # Filter 1: Check if dataset matches
        dataset_name = example.get("dataset", "")
        if dataset_name != dataset_filter:
            stats['filtered_wrong_dataset'] += 1
            continue
    ...
    print(f"  Filtered - wrong dataset: {stats['filtered_wrong_dataset']}")
    ...
    if len(processed_dataset) > 0:
        ...
        print(f"dataset: {ex.get('dataset', 'N/A')}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Process and clean the Aya unused dataset from hamishivi/tulu-3-unfiltered"
    )
    parser.add_argument(
        "--dataset-path",
        type=str,
        default="hamishivi/tulu-3-unfiltered",
        help="Path to the tulu-3-unfiltered dataset on HuggingFace Hub (default: hamishivi/tulu-3-unfiltered)",
    )
    parser.add_argument(
        "--dataset-split",
        type=str,
    ...
    parser.add_argument(
        "--dataset-filter",
        type=str,
        default="ai2-adapt-dev/tulu_v3.9_aya_unused",
        help="Dataset value to filter for (default: ai2-adapt-dev/tulu_v3.9_aya_unused)",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
