"""
Paraphrase script for swj0419/BookMIA dataset.

This script:
- Loads the BookMIA dataset
...
- Paraphrases specified fields (`snippet` by default) using Gemini via LiteLLM
- Copies originals to `snippet_original`
- Replaces paraphrased fields with paraphrased outputs
- Saves the resulting dataset to /gpfs/scrubbed/osey/Dataset_Distillation/data/bookmia-{size}-seed{seed}-paraphrased-v3.0
...
Usage:
  # Fresh run - paraphrase snippet field with random sampling
  python -m adra.dataset_paraphrase.paraphrase_bookmia \
      --subsample-size 64 \
      --subsample-seed 2
  
  # Fresh run - use pre-specified indices from file
  python -m adra.dataset_paraphrase.paraphrase_bookmia \
      --indices-file data/bookmia_rl/.../bookmia_64_indices.json
  ...
  python -m adra.dataset_paraphrase.paraphrase_bookmia \
    ...
    # continue from indieces
    python -m adra.dataset_paraphrase.paraphrase_bookmia \
    --indices-file /gpfs/scrubbed/osey/Dataset_Distillation/data/bookmia_rl/bookmia_64_rl_lexical_trio_v3_unique_ratio_penalty_1.50_augment_random_7_seed3_prefix_0.25_assist_0.25/bookmia_64_indices.json \
    --enable-batching \
    --batch-size 16
...
def save_sampled_indices(output_dir: Path, subsampled_dataset: Dataset, 
    ...
    indices_path = output_dir / f"bookmia_{subset_size}_indices.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Paraphrase swj0419/BookMIA dataset with required subsampling and checkpoint continuation support"
    )
    ...
    # Field selection
    parser.add_argument("--fields", nargs="+", default=["snippet"],
                        help="Fields to paraphrase (default: snippet)")
    ...
    parser.add_argument("--manual-paraphrase-file", type=str,
                        help="JSON file with manual paraphrases for entries blocked by content filters. Format: [{\"book_id\": X, \"snippet_id\": Y, \"snippet\": \"paraphrased text\"}]")
    ...
    if args.manual_paraphrase_file:
        ...
        if not manual_file_path.exists():
            ...
            # Create lookup dict by (book_id, snippet_id)
            for entry in manual_list:
                key = (entry['book_id'], entry['snippet_id'])
    ...
    if args.indices_file:
        ...
        indices_file_stem = indices_path.stem  # e.g., "bookmia_64_indices"
        ...
        output_dir = f"/gpfs/scrubbed/osey/Dataset_Distillation/data/bookmia-{subset_size}-seed{subset_seed}-paraphrased-v3.0"
        hf_repo_id = f"bookmia-{subset_size}-seed{subset_seed}-Paraphrased-Gemini-2.5-Flash-v3.0"
        sampled_indices_from_file = None
    
    # Configuration for BookMIA dataset
    ...
    if args.reprocess_extraction:
        ...
        # Add other BookMIA-specific fields from original dataset if they don't exist
        for field in ["book_id", "book", "snippet_id", "label"]:
