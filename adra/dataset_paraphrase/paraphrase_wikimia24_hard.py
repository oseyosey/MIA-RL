"""
Paraphrase script for hallisky/wikiMIA-2024-hard dataset.

This script:
- Loads the WikiMIA-2024-hard dataset
...
- Paraphrases specified fields (`input` by default) using Gemini via LiteLLM
- Copies originals to `input_original`
- Replaces paraphrased fields with paraphrased outputs
- Saves the resulting dataset to /gpfs/scrubbed/osey/Dataset_Distillation/data/wikimia24-hard-{size}-seed{seed}-paraphrased-v3.0
...
Usage:
  # Fresh run - paraphrase input field with random sampling
  python -m adra.dataset_paraphrase.paraphrase_wikimia24_hard \
      --subsample-size 64 \
      --subsample-seed 2
  
  # Fresh run - use pre-specified indices from file
  python -m adra.dataset_paraphrase.paraphrase_wikimia24_hard \
      --indices-file data/wikimia24_hard_rl/.../wikimia24_hard_64_indices.json
  ...
  # Continue from indices
  python -m adra.dataset_paraphrase.paraphrase_wikimia24_hard \
      --indices-file /gpfs/scrubbed/osey/Dataset_Distillation/data/wikimia24_hard_rl/wikimia24_hard_64_rl_lexical_trio_v3_unique_ratio_1.50_mia_adaptive_match_linear_distractor_max_augment_perturbed_3_seed1_prefix_0.25_assist_0.25_diff400_tokens256-1024/wikimia24_hard_64_indices.json \
      --enable-batching \
      --batch-size 16
...
def load_indices_from_file(indices_file: Path) -> tuple[List[int], Optional[int], Optional[int]]:
    ...
    logging.info("Loaded %d indices from file: %d members, %d non-members", 
                total_size, len(member_indices), len(nonmember_indices))
...
def subsample_dataset_from_indices(dataset: Dataset, sampled_indices: List[int]):
    ...
    logging.info("Subsampled %d / %d examples using provided indices", 
                len(sampled_indices), len(dataset))
    
    return dataset.select(sampled_indices), sampled_indices


def save_sampled_indices(output_dir: Path, subsampled_dataset: Dataset, 
                         sampled_indices: List[int], subset_size: int, 
                         subset_seed: int, dataset_path: str, split: str) -> None:
    """
    ...
    indices_path = output_dir / f"wikimia24_hard_{subset_size}_indices.json"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Paraphrase hallisky/wikiMIA-2024-hard dataset with required subsampling and checkpoint continuation support"
    )
    ...
    subsampling_group.add_argument("--indices-file", type=str,
                                   help="Path to JSON file containing member_indices and nonmember_indices (alternative to --subsample-size/--subsample-seed)")
    subsampling_group.add_argument("--subsample-size", type=int,
                                   help="Number of examples to subsample from dataset (required if not using --indices-file)")
    
    parser.add_argument("--subsample-seed", type=int,
                       help="Random seed for deterministic subsampling (required if not using --indices-file)")
    
    # Field selection
    parser.add_argument("--fields", nargs="+", default=["input"],
                        help="Fields to paraphrase (default: input)")
    ...
    parser.add_argument("--manual-paraphrase-file", type=str,
                        help="JSON file with manual paraphrases for entries blocked by content filters. Format: [{\"id\": X, \"input\": \"paraphrased text\"}]")
    ...
    if args.indices_file:
        ...
        sampled_indices_from_file, member_seed_from_file, total_size_from_file = load_indices_from_file(indices_path)
        ...
        indices_file_stem = indices_path.stem  # e.g., "wikimia24_hard_64_indices"
        output_dir = f"/gpfs/scrubbed/osey/Dataset_Distillation/data/{indices_file_stem}-paraphrased-v3.0"
        ...
        if args.subsample_size is None or args.subsample_seed is None:
            raise ValueError("Both --subsample-size and --subsample-seed are required when not using --indices-file")
        subset_size = args.subsample_size
        subset_seed = args.subsample_seed
        output_dir = f"/gpfs/scrubbed/osey/Dataset_Distillation/data/wikimia24-hard-{subset_size}-seed{subset_seed}-paraphrased-v3.0"
        hf_repo_id = f"wikimia24-hard-{subset_size}-seed{subset_seed}-Paraphrased-Gemini-2.5-Flash-v3.0"
        sampled_indices_from_file = None
    
    # Configuration for WikiMIA-2024-hard dataset
    dataset_path = "hallisky/wikiMIA-2024-hard"
    dataset_split = "test"
    ...
    if args.reprocess_extraction:
        ...
        # Add other WikiMIA-specific fields from original dataset if they don't exist
        for field in ["title", "date", "id", "diff", "label"]:
